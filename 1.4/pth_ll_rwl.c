...
{
   // Thread trying to acquire read lock
   pthread_mutex_lock(&lock->mutex);

   // Block readers if there are an other active
...
{
   // Thread trying to acquire write lock
   pthread_mutex_lock(&lock->mutex);

   // Don't write anything if there..
   // ..is any active reader or writer
   if (lock->active_writers > 0 || lock->active_readers > 0)
   {
...
int priority; // Checks if readers or writers have priority

// release lock from writing
void rwlock_release_write(rwlock_t *lock)
{
   ...
   // Checking which operation has priority
   if (priority == 1)
   {
      // If insert has priority..
      // ..make sure writers are activated first
   ...
   else
   {
      ...
      // ..make sure readers are activated first
...
int main(int argc, char *argv[])
{
   ...
   priority = atoi(argv[2]); // 1 for writers, 0 for readers

   inserts_in_main = atoi(argv[3]);
   total_ops = atoi(argv[4]);
   char *end;
   search_percent = strtod(argv[5], &end);
   insert_percent = strtod(argv[6], &end);

   delete_percent = 1.0 - (search_percent + insert_percent);

   if (priority != 1 && priority != 0)
   {
      printf("Wrong input, set default value (1)");
      priority = 1;
   }
...
void *Thread_work(void *rank)
{
   ...
   // If we cannot distribute equally the ops..
   // ..start distribute what's left to the first threads

   long long int moded_ops = total_ops % thread_count; // Remaining ops

   // Distribute the remainder to the first 'moded_ops' threads
   if (my_rank < moded_ops)
   {
      ops_per_thread++;
   }
